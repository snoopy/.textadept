...
local jump_functions = {
  [1] = function(reverse)
    util.move_to(jump_args, reverse)
  end,
  [2] = function(reverse)
    util.goto_zero_indent(reverse)
  end,
  [3] = function(reverse)
    util.goto_matching_indent(reverse)
  end,
}
...
local jump_hydra = hydra.create({
...

  {
    ...
    help = 'jump left',
    action = function()
      jump_functions[jump_action](true)
    end,
    persistent = true,
  },
  {
    ...
    action = function()
      ...
      for i = #_G._BUFFERS, 1, -1 do
        if _G._BUFFERS[i].filename then
          if _G._BUFFERS[i].filename:match(rootpath) then _G._BUFFERS[i]:close() end
        end
      end
    end,
  },
...
snippets.cpp.spdtrace = 'SPDLOG_LOGGER_TRACE(spdlog::default_logger(), "$0");'
snippets.cpp.spddbg = 'SPDLOG_LOGGER_DEBUG(spdlog::default_logger(), "$0");'
snippets.cpp.spdinfo = 'SPDLOG_LOGGER_INFO(spdlog::default_logger(), "$0");'
snippets.cpp.spdwarn = 'SPDLOG_LOGGER_WARN(spdlog::default_logger(), "$0");'
snippets.cpp.spderr = 'SPDLOG_LOGGER_ERROR(spdlog::default_logger(), "$0");'
snippets.cpp.spdcrit = 'SPDLOG_LOGGER_CRITICAL(spdlog::default_logger(), "$0");'
...
snippets.python.log = 'logging.${1:info}("${2:%s}", $0)'
